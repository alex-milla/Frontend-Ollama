...
(function () {
  ...
  function buildProjectCard(p) {
    ...
    card.innerHTML = `
      ...
      <div style="margin-top:12px;display:flex;gap:8px;">
        <a href="/chat?project_id=${p.id}" class="btn-sm" style="text-decoration:none;text-align:center;flex:1;">💬 Chatear</a>
      </div>`;
  ...
  async function openProjectModal(id) {
    ...
    if (all_skills.length === 0) {
      ...
      all_skills.forEach(s => {
        ...
        chip.style.cssText = `padding:4px 10px;border-radius:20px;font-size:.78rem;font-weight:600;cursor:pointer;border:1px solid ${active ? "var(--accent)" : "var(--border)"};background:${active ? "var(--accent-light)" : "var(--bg-secondary)"};color:${active ? "var(--accent)" : "var(--text-secondary)"};transition:all var(--transition);`;
  ...
  function toggleChip(chip) {
    ...
    chip.style.background = active ? "var(--accent-light)" : "var(--bg-secondary)";
